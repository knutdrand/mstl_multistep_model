...
| 12 | config_irs_chem | **code** | chemical-aware IRS decay: per-campaign decay half-life from the insecticide used (Bendiocarb 2 / Deltamethrin 3 / Actellic-Pirimiphos 5 / Fludora 8 mo) instead of fixed 4 mo | 0.32089 | 83.04 | — | — | **negative** — worse on both vs champion (0.3200/82.88). Per-chemical slopes are directionally sensible (Actellic/Fludora ≈ −0.23, Deltamethrin +0.13) but baking literature *rates* into the single decay feature over-persists protection. Motivates per-chemical *channels* (D2). Code inert (irs_chemical_column default None). |
| 13 | location_encoding (D1) | **code** | replace 406 sector one-hots with none / district(46) / KMeans-cluster(K) | — | — | — | — | **negative** (subset) — sector best (0.3307); none/district/cluster all ~0.002 worse. The dummies aren't dilutive noise: the RF uses them for location×climate interactions. Code inert (location_encoding="sector"). |
| 14 | config_irs_bank (D2) | **code** | expanded IRS feature bank: per-chemical decay channels + decay basis (2/8mo) + recency flags + rounds-12; let RF select | **0.31925** | **82.79** | — | — | **win** ✅ both metrics vs champion 0.3200/82.88. Perm-importance: organophosphate channel + decay2/8 + since/cumulative carry it; pyrethroid channel near-dead (resistance). |
| 15 | config_arima012 | **code** | **shared fixed ARIMA(0,1,2) order for all locations** instead of per-location AutoARIMA selection (user idea) | 0.31354 | 81.77 | — | — | **win** ✅✅ −2.0% log-CRPS / −1.3% CRPS vs config_tgtlag3_var — biggest single lever. Removes per-location order-selection noise (AutoARIMA overfits high-order/drift on short series). |
| 16 | config_arima012_bank | **code** | combine the two wins: shared ARIMA(0,1,2) + the IRS feature bank | **0.31232** | 81.88 | — | — | **CHAMPION** ✅ best log-CRPS — the two wins **stack** (vs 0.31354 for 012 alone). CRPS 81.88 (a hair above 012's 81.77; log-CRPS is primary). Ran pre-chap-upgrade, so comparable to the baselines. |
| — | order sweep (new chap) | — | shared order (0,1,1)/(1,1,1)/(0,1,3) — ran AFTER chap upgrade to master, so version-confounded vs the above | 0.31488 / 0.31502 / 0.31418 | 82.65 / 82.97 / **81.41** | — | — | among these (0,1,2) best log-CRPS; (0,1,3) best CRPS. NOT cleanly comparable to the old-chap rows — re-run on consistent chap to trust. |
